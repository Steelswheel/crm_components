<template>
    <div class="sber-promotions-table" id="sber-promotions">
        <grid
            controller="vaganov:lk"
            getRowsFn="promotionsRows"
            updateRowFn="promotionsUpdateRow"
        ...
        >
            ...
            <template #UF_IS_ACTIVE="{row}">
                <el-checkbox
                    ...
                    @change="changePromotionsState(row)"
                >
            ...
            <template #default="{row}">
                ...
                <div class="m-2">
                    <el-button
                        type="primary"
                        size="small"
                        @click="deletePromotionsItem(row.ID)"
                    >
                        Удалить
                    </el-button>
                </div>
...
import promotionsAddImage from './promotions-add-image';
import { BX_POST } from '@app/API';

export default {
    name: 'promotions-table',
    components: {
    ...
    methods: {
        ...
        changePromotionsState(row) {
            BX_POST('vaganov:lk', 'changePromotionsItemState', {
                id: row.ID,
        ...
        setPublicationDate(row) {
            BX_POST('vaganov:lk', 'changePromotionsPublicationDate', {id: row.ID, date: row.UF_PUBLICATION_DATE})
            .then(() => this.updateRow(row.ID))
            .catch(e => console.log(e));
        },
        deletePromotionsItem(id) {
            BX_POST('vaganov:lk', 'deletePromotionsItem', {id})
